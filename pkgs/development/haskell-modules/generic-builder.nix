...
let
  ...
  parallelismFlags = optionals enableParallelBuilding [
    "-j$NIX_BUILD_CORES"
    "--ghc-option=-j$NIX_BUILD_CORES"
  ];
  ...
  toolInputs = buildTools ++ libraryToolDepends ++ executableToolDepends;
  nativeBuildInputs = [ ghc removeReferencesTo ] ++ optional (allPkgconfigDepends != []) pkgconfig ++ toolInputs;
...
stdenv.mkDerivation ({
  ...
  setupCompilerEnvironmentPhase = ''
    ...
    for p in "''${__pkgsHostHost[@]}" "''${pkgsHostTarget[@]}"; do
  ...
  buildPhase = ''
    runHook preBuild
    ${setupCommand} build ${buildTarget} ${parallelismFlags}
    runHook postBuild
  '';
